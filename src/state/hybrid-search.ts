  ...
  private async tripleStreamSearch(
    ...
    const ranked = Array.from(scores.entries()).map(([obsId, s]) => {
      ...
      return {
        obsId,
        s,
        // The obs_ penalty must be applied HERE, before ranked.sort below, not where the
        // final rows are built: diversifyBySession walks its input in array order and never
        // re-sorts, so a penalty applied after the sort would change the reported score
        // without changing which rows win the slots.
        combinedScore:
          rrf *
          (1 + AGREEMENT_BONUS * (matchedStreams - 1)) *
